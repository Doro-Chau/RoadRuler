    ...
    for index, x in df_daily.iterrows():
        if index != 0:
            df_return = df_return.append(
                {
                    "date": df_daily.loc[index]["date"],
                    "cctv": df_daily.loc[index]["cctv"],
                    "park": df_daily.loc[index]["mongo_lot"]
                    - df_daily.loc[index - 1]["mongo_lot"],
                    "vd": df_daily.loc[index]["mongo_vd"]
                    - df_daily.loc[index - 1]["mongo_vd"],
                },
                ignore_index=True,
            )
...
def processmaplot(weekday, mondata):
    ...
    df_mondata = df_mondata.drop(columns=["_id"]).drop_duplicates(
        subset=["update_time"]
    )
    df_mondata[["weekday", "month", "date", "time", "CST", "year"]] = df_mondata[
        "update_time"
    ].str.split(" ", expand=True)
    df_mondata[["hr", "min", "sec"]] = df_mondata["time"].str.split(":", expand=True)
    df_mondata = (
        df_mondata.groupby(["hr", "weekday"])
        .agg({"totalcar": "mean", "availablecar": "mean"})
        .reset_index()
    )
    if (weekday != "平日") and (weekday != "假日"):
        df_mondata = df_mondata[df_mondata["weekday"] == weekday]
    elif weekday == "假日":
        df_mondata = df_mondata[
            (df_mondata["weekday"] == "Sat") | (df_mondata["weekday"] == "Sun")
        ]
...
def render_livevd(request):
    ...
    df_merge[["startlon", "startlat"]] = df_merge["startpoint"].str.split(
        ",", expand=True
    )
    ...
    df_merge = df_merge.drop(
        columns=["update_time_x", "update_time_y", "startpoint", "midpoint", "endpoint"]
    )
...
def render_alert(request):
    df_alert = pd.DataFrame(
        list(
            AlertLocation.objects.filter(
                expires__gt=(datetime.datetime.now() + datetime.timedelta(hours=8)),
                location__contains="臺北",
            ).values()
        )
    )
...
def render_construction(reuest):
    ...
    df_merge = df_construction.merge(
        df_construction_coor, how="inner", on="facility_no"
    )
    df_merge = df_merge[
        [
            "contractor",
...
def render_parking(request):
    ...
    df_parking = df_parking[
        ["id", "name", "totalcar", "availablecar", "entrancelat", "entrancelon"]
    ]
    df_parking[df_parking["availablecar"] != "無提供資料"] = df_parking[
        df_parking["availablecar"] != "無提供資料"
    ].astype({"availablecar": float})
    df_parking[df_parking["availablecar"] != "無提供資料"] = df_parking[
        df_parking["availablecar"] != "無提供資料"
    ].astype({"availablecar": int})
    df_parking[df_parking["availablecar"] != "無提供資料"] = df_parking[
        df_parking["availablecar"] != "無提供資料"
    ].astype({"availablecar": str})
    df_parking.loc[
        df_parking["availablecar"].str.contains("-", na=False), "availablecar"
    ] = "無提供資料"
