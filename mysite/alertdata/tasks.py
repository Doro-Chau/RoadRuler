...
def store_daily():
    ...
    b = models.MonitorDaily(
        date=today, cctv=cctv_amount, mongo_lot=lot_amount, mongo_vd=vd_amount
    )
    b.save()
    return "success"
...
def store_realtime():
    ...
    models.MonitorRealtime.objects.bulk_update(
        bulk_update, ["time", "average", "number", "realtime_amount"]
    )
    return "success"
    ...
    for d in data:
        if d["properties"]["Ac_no"] not in aclist:
            ...
            construction.append(
                models.Construction(
                    d["properties"]["Ac_no"],
                    d["properties"]["AppTime"],
                    d["properties"]["App_Name"],
                    d["properties"]["C_Name"],
                    d["properties"]["Addr"],
                    d["properties"]["Cb_Da"],
                    d["properties"]["Ce_Da"],
            ...
            if isinstance(d["properties"]["Positions"][i]["coordinates"][0], list):
                for coor in d["properties"]["Positions"][i]["coordinates"]:
                    coor_append = [
                        d["properties"]["Ac_no"],
                        i,
                        twd97_to_lonlat(coor[0], coor[1])[0],
                        twd97_to_lonlat(coor[0], coor[1])[1],
                    ]
                    if coor_append not in coorlist:
                        coorlist.append(coor_append)
                        constructioncoor.append(
                            models.ConstructionCoor(
                                d["properties"]["Ac_no"],
                                i,
                                twd97_to_lonlat(coor[0], coor[1])[0],
                                twd97_to_lonlat(coor[0], coor[1])[1],
                ...
                constructioncoor.append(
                    models.ConstructionCoor(
                        ...
                        i,
                        twd97_to_lonlat(
                            d["properties"]["Positions"][i]["coordinates"][0],
                            d["properties"]["Positions"][i]["coordinates"][1],
                        )[0],
                        twd97_to_lonlat(
                            d["properties"]["Positions"][i]["coordinates"][0],
                            d["properties"]["Positions"][i]["coordinates"][1],
...
def get_parking():
    ...
    for i in range(len(df_park)):
        df_park["entrancelat"][i], df_park["entrancelon"][i] = twd97_to_lonlat(
            df_park["tw97x"][i], df_park["tw97y"][i]
        )
    ...
    # insert into mongo
    df_mongo = df_merge[df_merge["update_time"].str.contains("CST")][
        ["id", "update_time", "totalcar", "availablecar"]
    ]
    db, client = get_db_handle(
        "traffic",
        os.getenv("MONGO_HOST"),
    ...
    models.Parkinglot.objects.bulk_update(
        bulk_update,
        [
            "update_time",
            "area",
...
def get_liveVD():
        ...
        for linkflow in vdlive["LinkFlows"]:
            for lane in linkflow["Lanes"]:
                ...
                # 3. update or create
                    ...
                    bulk_create.append(
                        [update_time, vdlive["VDID"], linkflow["LinkID"], lane["Speed"]]
                    )
                elif linkflow["LinkID"] in existing_link:
                    bulk_update.append(
                        [update_time, vdlive["VDID"], linkflow["LinkID"], lane["Speed"]]
                    )
    df_bulk_create = pd.DataFrame(
        bulk_create, columns=["update_time", "VDID", "LinkID", "Speed"]
    )
    df_bulk_update = pd.DataFrame(
        bulk_update, columns=["update_time", "VDID", "LinkID", "Speed"]
    )
    df_bulk_create = df_bulk_create[df_bulk_create["Speed"] != 0]
    df_bulk_update = df_bulk_update[df_bulk_update["Speed"] != 0]
    df_bulk_create = (
        df_bulk_create.groupby(["LinkID"])
        .agg({"update_time": "first", "Speed": "mean"})
        .reset_index()
    )
    df_bulk_update = (
        df_bulk_update.groupby(["LinkID"])
        .agg({"update_time": "first", "Speed": "mean"})
        .reset_index()
    )
    ...
    # bulk create
    df_bulk_create = [
        models.TrafficLivevd(x["update_time"], x["LinkID"], x["Speed"])
        for index, x in df_bulk_create.iterrows()
    ]
...
def get_link(url, LinkID):
    ...
    if data_link:
        insert = models.TrafficLink(
            data_link[0]["UpdateDate"],
            LinkID,
            data_link[0]["RoadName"],
            data_link[0]["StartPoint"],
            data_link[0]["MidPoint"],
            data_link[0]["EndPoint"],
        )
        insert.save()
...
def get_CCTV():
        ...
        if dict_CCTV["CCTVID"] not in cctvid:
            bulk_create.append(
                models.TrafficCctv(
                    update_time,
                    dict_CCTV["CCTVID"],
                    dict_CCTV["VideoStreamURL"],
                    dict_CCTV["PositionLon"],
                    dict_CCTV["PositionLat"],
                    dict_CCTV["RoadName"],
                )
            )
        else:
    ...
    models.TrafficCctv.objects.bulk_update(
        bulk_update,
        ["update_time", "videostreamurl", "positionlat", "positionlon", "roadname"],
    )
